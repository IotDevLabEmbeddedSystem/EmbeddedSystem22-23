...
def populateData(context: excel.RequestContext):
    ...
    r = context.workbook.worksheets.getItem("Sheet1").getCell(0, 0).getResizedRange(len(values) - 1, len(values[0]) - 1)
    r.clear()
    r.values = values
    r.worksheet.getRange("C:H").numberFormat = [["#,##0"]]
    r.worksheet.getRange("A:H").format.autofitColumns()
    table = r.worksheet.tables.add(r, True)
